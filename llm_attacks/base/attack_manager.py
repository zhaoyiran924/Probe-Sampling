...
from transformers import (AutoModelForCausalLM, AutoModelForSeq2SeqLM, AutoTokenizer, GPT2LMHeadModel,
                          GPTJForCausalLM, GPTNeoXForCausalLM, BitsAndBytesConfig, T5Model,
                          LlamaForCausalLM,)
import subprocess
import time
...
def track_memory_usage(interval=1):
    while True:
        ...
        print(f'Memory usage (MB): {memory_used}')
        time.sleep(interval)
    ...
    def logits(self, model, test_controls=None, return_ids=False):
        ...
        if return_ids:
            ...
            try:
                ...
                return model(input_ids=ids, attention_mask=attn_mask, decoder_input_ids=ids).logits, ids
        else:
            del locs, test_ids
    ...
    def run(self, 
        ...
        for i in range(n_steps):
            ...
            if self.logfile is not None and (i+1+anneal_from) % test_steps == 0:
                ...
                self.log(i+1+anneal_from, n_steps+anneal_from, last_control, loss, runtime, model_tests, verbose=verbose)
...
class ProgressiveMultiPromptAttack(object):
    ...
    def run(self, 
        ...
        if self.logfile is not None:
            ...
            log['params']['probe_set'] = probe_set
            log['params']['filtered_set'] = filtered_set
